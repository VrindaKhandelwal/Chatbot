...
def preprocess(raw_word):
    ...
    return filter_line(raw_word, EN_WHITELIST)
...
def generate_embedding_matrix (dictionary):
    ...
    print(f"loaded {len(embeddings_dict)} word embeddings, dimensions: {vector_size}")
    print ("generating embedding matrix...")

    embedding_matrix = np.zeros((len(dictionary)+1, vector_size[0]))
    for word, i in dictionary.items():
        if word in embeddings_dict: # words not found in embedding index will be all-zeros.
...
def process_data(FOLDER):
    ...
    print('\n>> Read lines from folder ' + FOLDER)
    FILENAME = f"{FOLDER}chat.txt"
    ...
    qtokenized = [ list ( filter (lambda x: x is not "", wordlist.split(' ')) ) for wordlist in qlines ]
    atokenized = [ list ( filter (lambda x: x is not "", wordlist.split(' ')) ) for wordlist in alines ]
    print('\n:: Sample from segmented list of words')
    print('\n q : {0};\n a : {1}'.format(qtokenized[60], atokenized[60]))
    print('\nq : {0};\n a : {1}'.format(qtokenized[61], atokenized[61]))

    # indexing -> idx2w, w2idx : en/ta
    print('\n >> Index words')
    idx2w, w2idx, freq_dist = index_( qtokenized + atokenized, vocab_size=VOCAB_SIZE)

    start_id = len(idx2w)  # 8002
    end_id = start_id + 1  # 8003

    w2idx.update({'start_id': start_id})
    w2idx.update({'end_id': end_id})
    idx2w = idx2w + ['start_id', 'end_id']

    embedding = generate_embedding_matrix (w2idx)
    ...
    # save them
    np.save(FOLDER + 'idx_q.npy', idx_q)
    np.save(FOLDER + 'idx_a.npy', idx_a)
